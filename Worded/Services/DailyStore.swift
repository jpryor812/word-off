...
final class DailyStore: ObservableObject {
    ...
    init() {
        Self.performOneTimeResetIfNeeded()
        load()
    }

    /// Clears every saved daily result (local history). Used for the one-time
    /// scoring/rack refresh so testers start with a clean slate.
    ...
    private static func performOneTimeResetIfNeeded() {
        ...
        let dir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
        try? FileManager.default.removeItem(at: dir.appendingPathComponent("daily_results.json"))
        UserDefaults.standard.set("", forKey: "worded.daily.unlocked")
    }
    ...
    func completedRackSizes(day: String) -> Set<Int> {
        Set(results.filter { $0.date == day }.map(\.rackSize))
    }

    func completedCount(day: String) -> Int {
